...
class GigForm extends Component{
    ...
    hanleSubmit = () => {
        ...
        firebase.firestore().collection("actorsgigs").add({jobcreater:uid,...this.state})
        .then((success) => {
          let key  = success.key;
          firebase.storage().ref(`gigsImages/${uid}/${uid}`).getDownloadURL().then(url => {
            firebase.firestore().collection("actorsgigs").doc(uid).set(
              {imgUrl:url}
            )
          console.log('gig added', success);
          })
        .catch((err) => {
          console.log(err);
        })
    }).catch(err=>console.log(err,'err'))
      // firebase.database().ref("gigPosts").push(
      //     this.state
      // ).then((success)=>{
      //   let key  = success.key;
      //   let uid = firebase.auth().currentUser.uid
      //   firebase.storage().ref(`gigImages/${uid}/${uid}`).getDownloadURL().then(url => {
      //     firebase.database().ref(`gigPosts/${key}`).update(
      //       {imgUrl:url}
      //   )   
      // })
      // })
    ...
    componentDidMount() {
      ...
      uploadImage = async (uri, imageName) => {
          ...
          var ref = firebase.storage().ref().child(`gigImages/${this.state.key}/` + imageName);
          return ref.put(blob);
      }
    ...
    render(){
        ...
        return(
            ...
            <ImageBackground source={signUp} style={styles.backgroundStyle}>
                ...
                <ScrollView style={{marginTop : 150, marginHorizontal : 15}}>
                ...
                <TextInput style={{...styles.textInputNames,width: "100%"}} placeholder="Tag Line" underlineColorAndroid={"transparent"} value={this.state.tagLine} onChangeText={(text) => this.setState({tagLine: text})} />
                ...
                <TouchableOpacity style={{paddingTop : 25}} onPress={this.hanleSubmit}>
                    <View style={styles.button}>
                        <Text style={{color : "white" , fontWeight : 'bold'}}> Post Gig </Text>
                    </View>
